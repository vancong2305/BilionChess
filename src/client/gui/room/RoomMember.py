...
def kick_player():
    if player_role.get() == "Chủ phòng":
        kick_label.config(text="Người chơi đã bị đá!")
    else:
        kick_label.config(text="Bạn không có quyền đá người chơi!")
...
square1_y = (canvas_height - square_size - 50) // 2
...
kick_label = tk.Label(window, text="Kick", font=("Arial", 14))
kick_label.place(relx=0.5, rely=0.5 + (2 * square_size + 50) / canvas_height, anchor=tk.CENTER)

kick_button = tk.Button(window, text="Kick Player", command=kick_player)
kick_button.place(relx=0.5, rely=0.5 + (2 * square_size + 100) / canvas_height, anchor=tk.CENTER)
